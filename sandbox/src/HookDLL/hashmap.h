...
static int
hashmap_create(const unsigned initial_size, struct hashmap_s *const out_hashmap) HASHMAP_USED;
...
static int
hashmap_put(struct hashmap_s *const hashmap, const void *const key, const unsigned len, void *const value) HASHMAP_USED;
...
static void *
hashmap_get(const struct hashmap_s *const hashmap, const void *const key, const unsigned len) HASHMAP_USED;
...
/// @return On success 0 is returned.
static int
hashmap_remove(struct hashmap_s *const hashmap, const void *const key, const unsigned len) HASHMAP_USED;
...
static void
hashmap_destroy(struct hashmap_s *const hashmap) HASHMAP_USED;

static unsigned
hashmap_crc32_helper(const char *const s, const unsigned len) HASHMAP_USED;
static unsigned
hashmap_hash_helper_int_helper(const struct hashmap_s *const m, const void *const keystring, const unsigned len)
    HASHMAP_USED;
static int
hashmap_match_helper(const struct hashmap_element_s *const element, const void *const key, const unsigned len)
    HASHMAP_USED;
static int
hashmap_hash_helper(
...
static int
hashmap_rehash_helper(struct hashmap_s *const m) HASHMAP_USED;
...
int
hashmap_create(const unsigned initial_size, struct hashmap_s *const out_hashmap)
{
    ...
    out_hashmap->data = HASHMAP_CAST(
        struct hashmap_element_s *,
        RtlAllocateHeap(RtlProcessHeap(), HEAP_ZERO_MEMORY, initial_size * sizeof(struct hashmap_element_s)));
    if (!out_hashmap->data)
    {
        return 1;
    }
...
int
hashmap_put(struct hashmap_s *const m, const void *const key, const unsigned len, void *const value)
{
    unsigned int index;
...
int
hashmap_remove(struct hashmap_s *const m, const char *const key, const unsigned len)
{
    unsigned int i;
    unsigned int curr;
...
int
hashmap_iterate(const struct hashmap_s *const m, int (*f)(void *const, void *const), void *const context)
{
    unsigned int i;
...
{
    RtlFreeHeap(RtlProcessHeap(), 0, m->data);
...
unsigned
hashmap_crc32_helper(const char *const s, const unsigned len)
{
    unsigned i;
    unsigned crc32val = 0;
    ...
    {
        crc32val =
            crc32_tab[(HASHMAP_CAST(unsigned char, crc32val) ^ HASHMAP_CAST(unsigned char, s[i]))] ^ (crc32val >> 8);
    }
    return crc32val;
#endif
}
...
hashmap_hash_helper_int_helper(const struct hashmap_s *const m, const void *const keystring, const unsigned len)
{
    ...
    {
		// String hashing.
        key = hashmap_crc32_helper((char *)keystring, len);
...
{
    if (len == 0)
		return (UINT)element->key == (UINT)key;
    return (element->key_len == len) && (len == RtlCompareMemory(element->key, key, len));
}
    ...
    unsigned *const out_index)
{
    ...
    for (i = 0; i < HASHMAP_MAX_CHAIN_LENGTH; i++)
    {
        ...
        if (m->data[curr].in_use && hashmap_match_helper(&m->data[curr], key, len))
        {
            *out_index = curr;
            return 1;
        }
